...
OneCore = AceLibrary("AceAddon-2.0"):new("AceEvent-2.0", "AceModuleCore-2.0", "AceHook-2.1")

local L = AceLibrary("AceLocale-2.1"):GetInstance("OneBag")
local LC = AceLibrary("AceLocale-2.1"):GetInstance("OneCore", true)
...
function OneCore:OnInitialize()
    ...
    self:Hook("GetContainerNumSlots", function(id)
        ...
        if id == -2 then
            ...
            return self.hooks.GetContainerNumSlots(id)
        end
    end)
